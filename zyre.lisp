...
(defun zpinger (&key interface background filter)
...
identical to the C library reference application.  If called with :filter t, returns a pipe
with zpinger messages filtered out so that further handling can be daisy-chained."
  (if background
      (bordeaux-threads:make-thread (lambda () (zpinger :interface interface)) :name "zpinger")
      (labels
          ((handler (x)
             (match x
               ...
               ((whisper-event (event-msg "Hello") (name n))
                ...
                t)
               (otherwise nil))))
        (let ((pipe (zyre-pipe :group "GLOBAL" :name (machine-instance) :interface interface)))
          (if filter
              (pipe-filter #'handler pipe)
              (pipe-mapc #'handler pipe))))))

(defun zyredir (&key (dir #P"/tmp/zyre/") interface background)
...
Removes files when peers exit. JSON data contains name, address, and headers, respectively."
  (if background
      (bordeaux-threads:make-thread (lambda () (zyredir :dir dir :interface interface))
                                    :name "zyredir")
      (labels
          ((json-f (u)
             (merge-pathnames (make-pathname :name u :type "json" :defaults dir) dir))
           (json-rm (&optional (l (directory (merge-pathnames
                                              (make-pathname :name :wild :type "json") dir))))
             (unless (endp l)
               (delete-file (car l))
               (json-rm (cdr l)))))
        ...
        (pipe-mapc
         ...
         (zyre-pipe :interface interface)))))

(defun zyre-daemon (&key (name (machine-instance)) headers interface)
  "Runs a simple zyre in the background for service discovery via name and headers. Ignores all
incoming events and messages.  Intended as a sample, may change behavior in future releases."
  (bordeaux-threads:make-thread
   (lambda () (pipe-sink (zyre-pipe :name name :headers headers :interface interface)))
   :name (concatenate 'string "zyre-daemon-" name)))
